...
public class Zone {
  ...
  public static void removeZone(String id) {
    ...
    for (Zone z : zones.values()) {
      ...
      zoneConf.set("zones." + zId + ".enabled", Boolean.valueOf(enabled));
    } 
    zoneConf.save();
  }
  ...
  public static void saveZone(String id) {
    ...
    zone.set("zones." + id + ".enabled", Boolean.valueOf(enabled));
    zone.save();
    if (z.getCuboid() == null && 
      z.getCorn1() != null && z.getCorn2() != null)
  ...
  public void giveCorns(Player player) {
    player.getInventory().addItem(new ItemStack[] { getCorn() });
  ...
  public boolean isInZone(Block block) {
    if (!this.active)
      return false; 
    return detectBlock(block, this.cuboid);
  }
