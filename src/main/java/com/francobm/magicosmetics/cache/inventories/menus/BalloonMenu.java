...
public class BalloonMenu extends PaginatedMenu {
  ...
  public void handleMenu(InventoryClickEvent event) {
    ...
    if (slotMenu == null)
      return; 
    if (slotMenu.getItems().getId().endsWith("_balloon") && 
    ...
    if (getNextSlot().contains(Integer.valueOf(slotMenu.getSlot()))) {
      int cosmetics = Cosmetic.getCosmeticCount(CosmeticType.BALLOON);
      slotMenu.playSound(player);
  ...
  public void setItems() {
    ...
    if (!cosmetics.isEmpty()) {
      ...
      for (int i = 0; i < getMaxItemsPerPage(); i++) {
        ...
        Items items = new Items("" + getPage() + this.index + "_balloon", Items.getItem("balloon-template").copyItem(this.playerData, cosmetic, cosmetic.getItemStack()));
        ...
        if (this.playerData.getBalloon() != null) {
          if (this.playerData.getBalloon().getId().equalsIgnoreCase(cosmetic.getId())) {
        ...
        setItemInPaginatedMenu(slotMenu, getPage(), this.index, "_balloon");
        continue;
      } 
    } 
