...
public class SprayMenu extends PaginatedMenu {
  ...
  public void handleMenu(InventoryClickEvent event) {
    ...
    if (getNextSlot().contains(Integer.valueOf(slotMenu.getSlot()))) {
      int cosmetics = Cosmetic.getCosmeticCount(CosmeticType.SPRAY);
      slotMenu.playSound(player);
  ...
  public void setItems() {
    ...
    if (!cosmetics.isEmpty()) {
      ...
      for (int i = 0; i < getMaxItemsPerPage(); i++) {
        ...
        Items items = new Items("" + getPage() + this.index + "_spray", Items.getItem("spray-template").copyItem(this.playerData, cosmetic, cosmetic.getItemStack()));
        ...
        if (this.playerData.getSpray() != null) {
          if (this.playerData.getSpray().getId().equalsIgnoreCase(cosmetic.getId())) {
        ...
        setItemInPaginatedMenu(slotMenu, getPage(), this.index, "_spray");
        continue;
      } 
    } 
