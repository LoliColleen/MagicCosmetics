...
public abstract class Menu implements InventoryHolder {
  ...
  public void open() {
    ...
    if (MagicCosmetics.getInstance().isPlaceholderAPI()) {
      getContentMenu().createInventory(this, MagicCosmetics.getInstance().getPlaceholderAPI().setPlaceholders(this.playerData.getOfflinePlayer().getPlayer(), getContentMenu().getTitle()));
    } else {
      getContentMenu().createInventory(this);
    } 
    this.playerData.getOfflinePlayer().getPlayer().openInventory(getInventory());
    setItems();
  }
  ...
  public static void loadMenus() {
    ...
    for (String key : menu.getConfigurationSection("menus").getKeys(false)) {
      ...
      SprayMenu sprayMenu;
      FreeMenu freeMenu;
      ColoredMenu coloredMenu;
      FreeColoredMenu freeColoredMenu;
      TokenMenu tokenMenu;
      ...
      for (String slot : menu.getConfigurationSection("menus." + key).getKeys(false)) {
        ...
        slotMenus.put(Integer.valueOf(itemSlot), new SlotMenu(itemSlot, item, commands, open_menu, sound, permission, actionType));
      } 
      ContentMenu contentMenu = new ContentMenu(title, size, inventoryType, slotMenus, previewSlot, resultSlot);
      switch (inventoryType) {
        case HAT:
          hatMenu = new HatMenu(key, contentMenu, startSlot, endSlot, backButton, nextButton, pagesSlot, slotsUnavailable);
          hatMenu.setPermission(perm);
          inventories.put(key, hatMenu);
          break;
        case BAG:
          bagMenu = new BagMenu(key, contentMenu, startSlot, endSlot, backButton, nextButton, pagesSlot, slotsUnavailable);
          bagMenu.setPermission(perm);
          inventories.put(key, bagMenu);
          break;
        case WALKING_STICK:
          wStickMenu = new WStickMenu(key, contentMenu, startSlot, endSlot, backButton, nextButton, pagesSlot, slotsUnavailable);
          wStickMenu.setPermission(perm);
          inventories.put(key, wStickMenu);
          break;
        case BALLOON:
          balloonMenu = new BalloonMenu(key, contentMenu, startSlot, endSlot, backButton, nextButton, pagesSlot, slotsUnavailable);
          balloonMenu.setPermission(perm);
          inventories.put(key, balloonMenu);
          break;
        case SPRAY:
          sprayMenu = new SprayMenu(key, contentMenu, startSlot, endSlot, backButton, nextButton, pagesSlot, slotsUnavailable);
          sprayMenu.setPermission(perm);
          inventories.put(key, sprayMenu);
          break;
        case FREE:
          freeMenu = new FreeMenu(key, contentMenu);
          freeMenu.setPermission(perm);
          inventories.put(key, freeMenu);
          break;
        case COLORED:
          coloredMenu = new ColoredMenu(key, contentMenu, startSlot, endSlot, backButton, nextButton, pagesSlot, slotsUnavailable);
          coloredMenu.setPermission(perm);
          inventories.put(key, coloredMenu);
          break;
        case FREE_COLORED:
          freeColoredMenu = new FreeColoredMenu(key, contentMenu, startSlot, endSlot, backButton, nextButton, pagesSlot, slotsUnavailable, containItem, unavailableColors);
          freeColoredMenu.setPermission(perm);
          inventories.put(key, freeColoredMenu);
          break;
        case TOKEN:
          tokenMenu = new TokenMenu(key, contentMenu, drag);
  ...
  public void setItemInMenu(SlotMenu slotMenu) {
    ...
    this.contentMenu.getInventory().setItem(slotMenu.getSlot(), slotMenu.getItems().getItemStack());
  }
  
  public void resetItems(List<Integer> ignoredSlots) {
    for (SlotMenu slots : getContentMenu().getSlotMenu().values()) {
      if (ignoredSlots.contains(Integer.valueOf(slots.getSlot())))
        continue; 
      this.contentMenu.getInventory().setItem(slots.getSlot(), XMaterial.AIR.parseItem());
    } 
    this.contentMenu.resetSlotMenu(ignoredSlots);
  }
  ...
  public void setItemInPaginatedMenu(SlotMenu slotMenu, int page, int index, String endsWith) {
    ...
    if (!slotMenu.getItems().getId().equalsIgnoreCase("" + page + index + page + index))
      return; 
    this.contentMenu.getInventory().setItem(slotMenu.getSlot(), slotMenu.getItems().getItemStack());
  }
  
  public Inventory getInventory() {
    return this.contentMenu.getInventory();
  }
  
  public String toString() {
    return "Menu{id='" + this.id + "', playerCache=" + String.valueOf(this.playerData) + ", contentMenu=" + String.valueOf(this.contentMenu) + "}";
  }
