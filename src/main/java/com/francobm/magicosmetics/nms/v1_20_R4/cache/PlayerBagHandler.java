...
public class PlayerBagHandler extends PlayerBag {
  ...
  public PlayerBagHandler(Player p, IRangeManager rangeManager, double distance, float height, ItemStack backPackItem, ItemStack backPackItemForMe) {
    ...
    this.backpackId = this.armorStand.al();
  ...
  public void spawnSelf(Player player) {
    ...
    if (this.height > 0.0F) {
      ...
      for (i = 0; i < this.height; i++) {
        ...
        this.ids.add(Integer.valueOf(entityAreaEffectCloud.al()));
      } 
      for (i = 0; i < this.height; i++) {
        if (i == 0) {
      ...
      addPassenger(player, ((Integer)this.ids.get(this.ids.size() - 1)).intValue(), this.armorStand.al());
    } else {
      addPassenger(player, (this.lendEntityId == -1) ? owner.getEntityId() : this.lendEntityId, this.armorStand.al());
    } 
  ...
  public void addPassenger(boolean exception) {
    List<Packet<?>> backPack = getBackPackMountPacket((this.lendEntityId == -1) ? getPlayer().getEntityId() : this.lendEntityId, this.armorStand.al());
    for (Player player : getPlayersInRange()) {
      if (exception && player.getUniqueId().equals(this.uuid))
        continue; 
  ...
  private List<Packet<?>> getBackPackSpawn(ItemStack backpackItem) {
    ...
    PacketPlayOutEntityMetadata entityMetadata = new PacketPlayOutEntityMetadata(this.armorStand.al(), this.armorStand.ap().c());
    PacketPlayOutMount mountEntity = new PacketPlayOutMount((Entity)this.entityPlayer);
    PacketPlayOutEntityEquipment equip = new PacketPlayOutEntityEquipment(this.armorStand.al(), Collections.singletonList(new Pair(EnumItemSlot.f, CraftItemStack.asNMSCopy(backpackItem))));
  ...
  private List<Packet<?>> getBackPackDismount(boolean removeClouds) {
    ...
    if (!removeClouds) {
      PacketPlayOutEntityDestroy backPackDestroy = new PacketPlayOutEntityDestroy(new int[] { this.armorStand.al() });
      return (List)Collections.singletonList(backPackDestroy);
    } 
    for (Integer id : this.ids) {
      packets.add(new PacketPlayOutEntityDestroy(new int[] { id.intValue() }));
    } 
    packets.add(new PacketPlayOutEntityDestroy(new int[] { this.armorStand.al() }));
    return packets;
  }
  ...
  private List<Packet<?>> getBackPackHelmetPacket(ItemStack itemStack) {
    ...
    return (List)Collections.singletonList(new PacketPlayOutEntityEquipment(this.armorStand.al(), list));
  }
  
  private List<Packet<?>> getBackPackHelmetPacket(ArrayList<Pair<EnumItemSlot, ItemStack>> pairs) {
    return (List)Collections.singletonList(new PacketPlayOutEntityEquipment(this.armorStand.al(), pairs));
  }
  ...
  private List<Packet<?>> getBackPackRotationPackets(float yaw) {
    ...
    PacketPlayOutEntity.PacketPlayOutEntityLook packetPlayOutEntityLook = new PacketPlayOutEntity.PacketPlayOutEntityLook(this.armorStand.al(), (byte)(int)(yaw * 256.0F / 360.0F), (byte)0, true);
