...
public class CustomSprayHandler extends CustomSpray {
  ...
  public CustomSprayHandler(Player player, Location location, BlockFace blockFace, ItemStack itemStack, MapView mapView, int rotation) {
    ...
    this.itemFrame = new EntityItemFrame(EntityTypes.ai, (World)world);
  ...
  public void spawn(Player player) {
    ...
    if (!player.getWorld().equals(this.location.getWorld()))
      return; 
    this.itemFrame.k(true);
    this.itemFrame.n(true);
  ...
  private List<Packet<?>> spawnItemFrame() {
    PacketPlayOutSpawnEntity spawnEntity = new PacketPlayOutSpawnEntity((Entity)this.itemFrame, this.enumDirection.d(), CraftLocation.toBlockPosition(this.location));
    PacketPlayOutEntityMetadata entityMetadata = new PacketPlayOutEntityMetadata(this.itemFrame.an(), this.itemFrame.ar().c());
    return Arrays.asList((Packet<?>[])new Packet[] { (Packet)spawnEntity, (Packet)entityMetadata });
  }
  
  private Packet<?> destroyItemFrame() {
    return (Packet<?>)new PacketPlayOutEntityDestroy(new int[] { this.itemFrame.an() });
  }
