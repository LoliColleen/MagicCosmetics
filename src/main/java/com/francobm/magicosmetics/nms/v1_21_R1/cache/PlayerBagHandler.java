...
public class PlayerBagHandler extends PlayerBag {
  ...
  public PlayerBagHandler(Player p, IRangeManager rangeManager, double distance, float height, ItemStack backPackItem, ItemStack backPackItemForMe) {
    ...
    this.backpackId = this.armorStand.an();
  ...
  public void spawnSelf(Player player) {
    ...
    if (this.height > 0.0F) {
      ...
      for (i = 0; i < this.height; i++) {
        ...
        this.ids.add(Integer.valueOf(entityAreaEffectCloud.an()));
      } 
      for (i = 0; i < this.height; i++) {
        if (i == 0) {
      ...
      addPassenger(player, ((Integer)this.ids.get(this.ids.size() - 1)).intValue(), this.armorStand.an());
    } else {
      addPassenger(player, (this.lendEntityId == -1) ? owner.getEntityId() : this.lendEntityId, this.armorStand.an());
    } 
  ...
  public void addPassenger(boolean exception) {
    List<Packet<?>> backPack = getBackPackMountPacket((this.lendEntityId == -1) ? getPlayer().getEntityId() : this.lendEntityId, this.armorStand.an());
    for (Player player : getPlayersInRange()) {
      if (exception && player.getUniqueId().equals(this.uuid))
        continue; 
  ...
  private List<Packet<?>> getBackPackSpawn(ItemStack backpackItem) {
    PacketPlayOutSpawnEntity spawnEntity = new PacketPlayOutSpawnEntity((Entity)this.armorStand, 0, CraftLocation.toBlockPosition(this.entityPlayer.getBukkitEntity().getLocation()));
    PacketPlayOutEntityMetadata entityMetadata = new PacketPlayOutEntityMetadata(this.armorStand.an(), this.armorStand.ar().c());
    PacketPlayOutEntityEquipment equip = new PacketPlayOutEntityEquipment(this.armorStand.an(), Collections.singletonList(new Pair(EnumItemSlot.f, CraftItemStack.asNMSCopy(backpackItem))));
  ...
  private List<Packet<?>> getCloudsSpawn(EntityAreaEffectCloud entityAreaEffectCloud) {
    PacketPlayOutSpawnEntity spawnEntity = new PacketPlayOutSpawnEntity((Entity)entityAreaEffectCloud, 0, CraftLocation.toBlockPosition(entityAreaEffectCloud.getBukkitEntity().getLocation()));
    PacketPlayOutEntityMetadata entityMetadata = new PacketPlayOutEntityMetadata(entityAreaEffectCloud.an(), entityAreaEffectCloud.ar().c());
  ...
  private List<Packet<?>> getBackPackDismount(boolean removeClouds) {
    ...
    if (!removeClouds) {
      PacketPlayOutEntityDestroy backPackDestroy = new PacketPlayOutEntityDestroy(new int[] { this.armorStand.an() });
      return (List)Collections.singletonList(backPackDestroy);
    } 
    for (Integer id : this.ids) {
      packets.add(new PacketPlayOutEntityDestroy(new int[] { id.intValue() }));
    } 
    packets.add(new PacketPlayOutEntityDestroy(new int[] { this.armorStand.an() }));
    return packets;
  }
  ...
  private List<Packet<?>> getBackPackHelmetPacket(ItemStack itemStack) {
    ...
    return (List)Collections.singletonList(new PacketPlayOutEntityEquipment(this.armorStand.an(), list));
  }
  
  private List<Packet<?>> getBackPackHelmetPacket(ArrayList<Pair<EnumItemSlot, ItemStack>> pairs) {
    return (List)Collections.singletonList(new PacketPlayOutEntityEquipment(this.armorStand.an(), pairs));
  }
  ...
  private List<Packet<?>> getBackPackRotationPackets(float yaw) {
    ...
    PacketPlayOutEntity.PacketPlayOutEntityLook packetPlayOutEntityLook = new PacketPlayOutEntity.PacketPlayOutEntityLook(this.armorStand.an(), (byte)(int)(yaw * 256.0F / 360.0F), (byte)0, true);
