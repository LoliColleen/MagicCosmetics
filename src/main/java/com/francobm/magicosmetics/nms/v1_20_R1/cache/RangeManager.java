...
public class RangeManager implements IRangeManager {
  ...
  public void addPlayer(Player player) {
    this.tracked.f.add((((CraftPlayer)player).getHandle()).c);
  }
  
  public void removePlayer(Player player) {
    this.tracked.f.remove((((CraftPlayer)player).getHandle()).c);
  }
  ...
  public Set<Player> getPlayerInRange() {
    ...
    if (this.tracked == null)
      return list; 
    this.tracked.f.forEach(serverPlayerConnection -> list.add(serverPlayerConnection.f().getBukkitEntity()));
    return list;
  }
}


/* Location:              D:\下载\MagicCosmetics-3.1.0[tinksp.com].jar!\com\francobm\magicosmetics\nms\v1_20_R1\cache\RangeManager.class
