...
public class MCChannelHandler extends ChannelDuplexHandler {
  ...
  public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
    if (msg instanceof PacketPlayOutSetSlot) {
      ...
      for (Packet<?> subPacket : (Iterable<Packet<?>>)packet.a()) {
        if (subPacket instanceof PacketPlayOutNamedEntitySpawn) {
          PacketPlayOutNamedEntitySpawn otherPacket = (PacketPlayOutNamedEntitySpawn)subPacket;
          handleEntitySpawn(otherPacket.a());
          continue;
        } 
        if (subPacket instanceof PacketPlayOutEntityDestroy) {
          PacketPlayOutEntityDestroy otherPacket = (PacketPlayOutEntityDestroy)subPacket;
          for (IntListIterator<Integer> intListIterator = otherPacket.a().iterator(); intListIterator.hasNext(); ) {
  ...
  protected Entity getEntityAsync(WorldServer world, int id) {
    Entity entity = (Entity)getEntityGetter(world).a(id);
  ...
  public static LevelEntityGetter<Entity> getEntityGetter(WorldServer level) {
    if (entityGetter == null)
      return level.L.d(); 
    try {
      return (LevelEntityGetter<Entity>)entityGetter.invoke(level, new Object[0]);
...
/* Location:              D:\下载\MagicCosmetics-3.1.0[tinksp.com].jar!\com\francobm\magicosmetics\nms\v1_19_R3\models\MCChannelHandler.class
