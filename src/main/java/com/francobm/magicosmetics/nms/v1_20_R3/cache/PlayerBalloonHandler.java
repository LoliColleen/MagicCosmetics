...
public class PlayerBalloonHandler extends PlayerBalloon {
  ...
  public PlayerBalloonHandler(Player p, double space, double distance, boolean bigHead, boolean invisibleLeash) {
    ...
    this.leashed = (EntityLiving)new EntityPufferFish(EntityTypes.aC, (World)world);
  ...
  public void spawn(Player player) {
    ...
    this.armorStand.an().refresh(entityPlayer);
    entityPlayer.c.b((Packet)new PacketPlayOutSpawnEntity((Entity)this.leashed));
    this.leashed.an().refresh(entityPlayer);
    if (!this.invisibleLeash)
      entityPlayer.c.b((Packet)new PacketPlayOutAttachEntity((Entity)this.leashed, (this.lendEntity == null) ? (Entity)((CraftPlayer)owner).getHandle() : (Entity)((CraftLivingEntity)this.lendEntity).getHandle())); 
  ...
  public void remove(Player player) {
    ...
    connection.b((Packet)new PacketPlayOutEntityDestroy(new int[] { this.armorStand.aj() }));
    connection.b((Packet)new PacketPlayOutEntityDestroy(new int[] { this.leashed.aj() }));
    connection.b((Packet)new PacketPlayOutEntityDestroy(new int[] { this.leashed.aj() }));
  ...
  public void setItem(ItemStack itemStack) {
    ...
    for (UUID uuid : this.viewers) {
      ...
      PlayerConnection connection = (((CraftPlayer)player).getHandle()).c;
      connection.b((Packet)new PacketPlayOutEntityEquipment(this.armorStand.aj(), list));
    } 
  }
  ...
  public void lookEntity(float yaw, float pitch) {
    for (UUID uuid : this.viewers) {
      ...
      connection.b((Packet)new PacketPlayOutEntity.PacketPlayOutEntityLook(this.armorStand.aj(), (byte)(int)(yaw * 256.0F / 360.0F), (byte)0, true));
      connection.b((Packet)new PacketPlayOutEntityHeadRotation((Entity)this.leashed, (byte)(int)(yaw * 256.0F / 360.0F)));
      connection.b((Packet)new PacketPlayOutEntity.PacketPlayOutEntityLook(this.leashed.aj(), (byte)(int)(yaw * 256.0F / 360.0F), (byte)0, true));
    } 
  }
  ...
  protected void instantUpdate() {
    ...
    if (!this.rotateLoop) {
      this.rot += 0.02D;
      this.armorStand.a(new Vector3f(this.armorStand.B().b() - 0.5F, this.armorStand.B().c(), this.armorStand.B().d() + this.rotate));
      if (this.rot > 0.2D)
    ...
    if (this.heightLoop) {
      this.height -= 0.01D;
      this.armorStand.a(new Vector3f(this.armorStand.B().b() + 0.8F, this.armorStand.B().c(), this.armorStand.B().d()));
      if (this.height < -0.1D)
    ...
    for (UUID uuid : this.viewers) {
      ...
      if (!this.invisibleLeash)
        p.c.b((Packet)new PacketPlayOutAttachEntity((Entity)this.leashed, (this.lendEntity == null) ? (Entity)((CraftPlayer)owner).getHandle() : (Entity)((CraftLivingEntity)this.lendEntity).getHandle())); 
      this.armorStand.an().refresh(p);
  ...
  public void rotate(boolean rotation, RotationType rotationType, float rotate) {
    ...
    switch (rotationType) {
      case RIGHT:
        this.armorStand.a(new Vector3f(this.armorStand.B().b(), this.armorStand.B().c() + rotate, this.armorStand.B().d()));
        break;
      case UP:
        this.armorStand.a(new Vector3f(this.armorStand.B().b() + rotate, this.armorStand.B().c(), this.armorStand.B().d()));
        break;
      case ALL:
        this.armorStand.a(new Vector3f(this.armorStand.B().b() + rotate, this.armorStand.B().c() + rotate, this.armorStand.B().d()));
        break;
    } 
    for (UUID uuid : this.viewers) {
      Player player = Bukkit.getPlayer(uuid);
      ...
      this.armorStand.an().refresh(((CraftPlayer)player).getHandle());
    } 
  }
