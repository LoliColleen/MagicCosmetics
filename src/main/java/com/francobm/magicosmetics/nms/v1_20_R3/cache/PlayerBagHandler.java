...
public class PlayerBagHandler extends PlayerBag {
  ...
  public PlayerBagHandler(Player p, IRangeManager rangeManager, double distance, float height, ItemStack backPackItem, ItemStack backPackItemForMe) {
    ...
    this.backpackId = this.armorStand.aj();
  ...
  public void spawnSelf(Player player) {
    ...
    if (this.height > 0.0F) {
      ...
      for (i = 0; i < this.height; i++) {
        ...
        this.ids.add(Integer.valueOf(entityAreaEffectCloud.aj()));
      } 
      for (i = 0; i < this.height; i++) {
        if (i == 0) {
      ...
      addPassenger(player, ((Integer)this.ids.get(this.ids.size() - 1)).intValue(), this.armorStand.aj());
    } else {
      addPassenger(player, (this.lendEntityId == -1) ? owner.getEntityId() : this.lendEntityId, this.armorStand.aj());
    } 
  ...
  public void addPassenger(boolean exception) {
    List<Packet<?>> backPack = getBackPackMountPacket((this.lendEntityId == -1) ? getPlayer().getEntityId() : this.lendEntityId, this.armorStand.aj());
    for (Player player : getPlayersInRange()) {
      if (exception && player.getUniqueId().equals(this.uuid))
        continue; 
  ...
  private List<Packet<?>> getBackPackSpawn(ItemStack backpackItem) {
    ...
    PacketPlayOutEntityMetadata entityMetadata = new PacketPlayOutEntityMetadata(this.armorStand.aj(), this.armorStand.an().c());
    PacketPlayOutMount mountEntity = new PacketPlayOutMount((Entity)this.entityPlayer);
    PacketPlayOutEntityEquipment equip = new PacketPlayOutEntityEquipment(this.armorStand.aj(), Collections.singletonList(new Pair(EnumItemSlot.f, CraftItemStack.asNMSCopy(backpackItem))));
  ...
  private List<Packet<?>> getCloudsSpawn(EntityAreaEffectCloud entityAreaEffectCloud) {
    ...
    PacketPlayOutEntityMetadata entityMetadata = new PacketPlayOutEntityMetadata(entityAreaEffectCloud.aj(), entityAreaEffectCloud.an().c());
  ...
  private List<Packet<?>> getBackPackDismount(boolean removeClouds) {
    ...
    if (!removeClouds) {
      PacketPlayOutEntityDestroy backPackDestroy = new PacketPlayOutEntityDestroy(new int[] { this.armorStand.aj() });
      return (List)Collections.singletonList(backPackDestroy);
    } 
    for (Integer id : this.ids) {
      packets.add(new PacketPlayOutEntityDestroy(new int[] { id.intValue() }));
    } 
    packets.add(new PacketPlayOutEntityDestroy(new int[] { this.armorStand.aj() }));
    return packets;
  }
  ...
  private List<Packet<?>> getBackPackHelmetPacket(ItemStack itemStack) {
    ...
    return (List)Collections.singletonList(new PacketPlayOutEntityEquipment(this.armorStand.aj(), list));
  }
  
  private List<Packet<?>> getBackPackHelmetPacket(ArrayList<Pair<EnumItemSlot, ItemStack>> pairs) {
    return (List)Collections.singletonList(new PacketPlayOutEntityEquipment(this.armorStand.aj(), pairs));
  }
  ...
  private List<Packet<?>> getBackPackRotationPackets(float yaw) {
    ...
    PacketPlayOutEntity.PacketPlayOutEntityLook packetPlayOutEntityLook = new PacketPlayOutEntity.PacketPlayOutEntityLook(this.armorStand.aj(), (byte)(int)(yaw * 256.0F / 360.0F), (byte)0, true);
