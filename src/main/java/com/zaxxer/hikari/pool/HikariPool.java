...
public final class HikariPool extends PoolBase implements HikariPoolMXBean, ConcurrentBag.IBagStateListener {
  ...
  private final long aliveBypassWindowMs = Long.getLong("com.zaxxer.hikari.aliveBypassWindowMs", TimeUnit.MILLISECONDS.toMillis(500L)).longValue();
  
  private final long housekeepingPeriodMs = Long.getLong("com.zaxxer.hikari.housekeeping.periodMs", TimeUnit.SECONDS.toMillis(30L)).longValue();
  ...
  public Connection getConnection(long hardTimeout) throws SQLException {
    ...
    try {
      ...
      do {
        ...
        if (poolEntry.isMarkedEvicted() || (ClockSource.elapsedMillis(poolEntry.lastAccessed, now) > this.aliveBypassWindowMs && isConnectionDead(poolEntry.connection))) {
  ...
  public synchronized void shutdown() throws InterruptedException {
    try {
      ...
      ThreadPoolExecutor assassinExecutor = UtilityElf.createThreadPoolExecutor(this.config.getMaximumPoolSize(), this.poolName + " connection assassinator", this.config
  ...
  public void addBagItem(int waiting) {
    ...
    if (countToAdd >= 0) {
      ...
      this.logger.debug("{} - Add connection elided, waiting={}, adders pending/running={}", new Object[] { this.poolName, Integer.valueOf(waiting), Integer.valueOf(queueDepth) });
    } 
  }
  ...
  private synchronized void fillPool(boolean isAfterAdd) {
    ...
    boolean shouldAdd = (getTotalConnections() < this.config.getMaximumPoolSize() && (getIdleConnections() < this.config.getMinimumIdle() || countToAdd > getIdleConnections()));
    if (shouldAdd) {
      this.addConnectionQueueDepth.incrementAndGet();
      this.addConnectionExecutor.submit(isAfterAdd ? this.postFillPoolEntryCreator : this.poolEntryCreator);
    } else if (isAfterAdd) {
      this.logger.debug("{} - Fill pool skipped, pool has sufficient level or currently being filled (queueDepth={}).", this.poolName, Integer.valueOf(queueDepth));
    } 
  }
  ...
  private SQLException createTimeoutException(long startTime) {
    ...
    Exception originalException = getLastConnectionFailure();
    if (originalException instanceof SQLException)
      sqlState = ((SQLException)originalException).getSQLState(); 
    SQLTransientConnectionException connectionException = new SQLTransientConnectionException(this.poolName + " - Connection is not available, request timed out after " + this.poolName + "ms.", sqlState, originalException);
    if (originalException instanceof SQLException)
  ...
  private final class PoolEntryCreator implements Callable<Boolean> {
        ...
        while (shouldContinueCreating()) {
          ...
          if (poolEntry != null) {
            ...
            if (this.loggingPrefix != null)
              HikariPool.this.logger.debug("{} - Connection add failed, sleeping with backoff: {}ms", HikariPool.this.poolName, Long.valueOf(backoffMs)); 
          } 
    ...
    private synchronized boolean shouldContinueCreating() {
      return (HikariPool.this.poolState == 0 && HikariPool.this.getTotalConnections() < HikariPool.this.config.getMaximumPoolSize() && (HikariPool.this
        .getIdleConnections() < HikariPool.this.config.getMinimumIdle() || HikariPool.this.connectionBag.getWaitingThreadCount() > HikariPool.this.getIdleConnections()));
    }
  }
